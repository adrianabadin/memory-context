...
async function buildCodeHash(lines, startLine, endLine) {
  const fragment = lines.slice(startLine - 1, endLine).join('\n');
  return hashSymbol(fragment);
}
...
async function symbolFromNode(node, lines, filePath, language, kind, name, arity) {
  const startLine = node.loc?.start?.line ?? 1;
  const endLine = node.loc?.end?.line ?? startLine;
  const codeHash = await buildCodeHash(lines, startLine, endLine);
...
export async function extractJsTsSymbols({ filePath, content }) {
  ...
  for (const node of ast.program.body) {
    ...
    switch (decl?.type) {
      ...
      case 'ClassExpression':
        if (decl.id?.name) {
          symbolPromises.push(symbolFromNode(decl, lines, filePath, language, 'class', decl.id.name, undefined));
        }
        break;
      ...
      case 'FunctionDeclaration':
        if (decl.id?.name) {
          ...
          symbolPromises.push(symbolFromNode(decl, lines, filePath, language, 'function', decl.id.name, arity));
        }
        break;
      ...
      case 'VariableDeclaration':
        for (const declarator of decl.declarations ?? []) {
          ...
          if (isArrow && declarator.id?.name) {
            ...
            symbolPromises.push(symbolFromNode(declarator, lines, filePath, language, 'function', declarator.id.name, arity));
          }
        }
        break;
      ...
      case 'TSInterfaceDeclaration':
        if (decl.id?.name) {
          symbolPromises.push(symbolFromNode(decl, lines, filePath, language, 'interface', decl.id.name, undefined));
        }
        break;

      case 'TSTypeAliasDeclaration':
        if (decl.id?.name) {
          symbolPromises.push(symbolFromNode(decl, lines, filePath, language, 'type', decl.id.name, undefined));
        }
        break;
