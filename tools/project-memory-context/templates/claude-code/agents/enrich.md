...
**Dispatch first 3 subagents in parallel** (`subagent_type: general-purpose`, `run_in_background: true`):
```
...
Report counts (Ollama enriched, subagents enriched, errors). Suggest `{{PMC_BIN}} sync-context`.
