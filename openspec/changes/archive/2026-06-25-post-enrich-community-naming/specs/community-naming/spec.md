...
Community names stored in graph.db MUST be returned by `pmc get-context` and displayed in the graph explorer in place of generic community IDs.
...
#### Scenario: Graph explorer renders communities

- GIVEN the graph explorer lists communities
- WHEN names are present in graph.db
- THEN it MUST display descriptive names instead of numeric IDs
